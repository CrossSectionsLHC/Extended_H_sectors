...
decay = "hbbhbb"
MX = 400
masses = [MX] # [260, 270, 280, 300, 320, 350, 400, 450, 500, 550, 600, 650, 700, 800, 900, 1000]
...
GoM = 0.001 
resonant     = table_res_xsec.loc[(table_res_xsec["GoM"] == GoM) & (table_res_xsec["Mass[Gev]"] == MX)]["sigma[pb]_SChan_eta0"].values[0]  #BR_HH === to be treated on the physics model  
...
duplicate = {
    "sig_NMSSM_bbbb_MX_%s_MY_125" % MX : {
    #"Radionhh500_2018" : {
        ...
        "ggHH_kl_0_kt_0_kX_1_ktX_1_restriangle_0_resbox_0_nonresonly_0" : resonant*br_HH_to_factor,
...
for mass in masses :
    ...
    for cat in catList:
        ...
        for prc in procList:
            ...
            # ["Radionhh260_2016"]
            procs_to_clone = ["sig_NMSSM_bbbb_MX_%s_MY_125" % MX]
            if prc in procs_to_clone:
                ...
                for target_prc in duplicate[proc_]:
                    ...
                    ch.CloneProcsAndSysts(
                        ...
                        lambda x: x.set_process(x.process().replace("sig_NMSSM_bbbb_MX_%s_MY_125" % MX, target_prc_name))
                    )
                    #    #cb.deep().bin([cat]).process([prc]),

                    def updateRate(x):
                        if x.process()==target_proc:
                            rate_now = x.rate()
                            print("     > rate", rate_now, target_rate, rate_now*target_rate, target_proc)
